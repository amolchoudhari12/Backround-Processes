...
{
    public class LocationMarketListTranslator : DataAccessTranslate<location_marketlist_translate>
    {

        protected override void SelectRecords()
        {
            using (var action = new LocationMarketListTranslatorActions())
            {
        ...
        protected override location_marketlist_translate ConvertListOfStringToEntity(List<string> translatedStrings)
        {
            location_marketlist_translate translatedAirportList = null;
            ...
            {
                translatedAirportList = new location_marketlist_translate()
                {
                    MarketName = translatedStrings[0],
                    AboutMarket = translatedStrings[1],
                };
            }
            return translatedAirportList;
        }

        protected override List<string> ConvertToListOfString(location_marketlist_translate inputEntity)
        {
            inputsForTranslator.Clear();
            inputsForTranslator.Add(inputEntity.MarketName);
            inputsForTranslator.Add(inputEntity.AboutMarket);
            return inputsForTranslator;
        }

        protected override void TranslateToOtherLanguages(location_marketlist_translate item, int sourceLanaguge)
        {
            foreach (var languageToTranslate in languagesSupported)
            {
                if (languageToTranslate != sourceLanaguge)
                {
                    ...
                    location_marketlist_translate translatedEntity = ConvertListOfStringToEntity(translatedStrings.ToList());

                    if (translatedEntity != null)
                    {
                        translatedEntity.LanguageID = languageToTranslate;
                        translatedEntity.MarketID = item.MarketID;
                    }
