...
{
    public class LocationMallListTranslator : DataAccessTranslate<location_malllist_translate>
    {

        protected override void SelectRecords()
        {
            using (var action = new LocationMallListTranslatorActions())
            {
        ...
        protected override location_malllist_translate ConvertListOfStringToEntity(List<string> translatedStrings)
        {
            location_malllist_translate translatedAirportList = null;
            ...
            {
                translatedAirportList = new location_malllist_translate()
                {
                    MallName = translatedStrings[0],
                    AboutMall= translatedStrings[1],
                };
            }
            return translatedAirportList;
        }

        protected override List<string> ConvertToListOfString(location_malllist_translate inputEntity)
        {
            inputsForTranslator.Clear();
            inputsForTranslator.Add(inputEntity.MallName);
            inputsForTranslator.Add(inputEntity.AboutMall);
            return inputsForTranslator;
        }

        protected override void TranslateToOtherLanguages(location_malllist_translate item, int sourceLanaguge)
        {
            foreach (var languageToTranslate in languagesSupported)
            {
                if (languageToTranslate != sourceLanaguge)
                {
                    ...
                    location_malllist_translate translatedEntity = ConvertListOfStringToEntity(translatedStrings.ToList());

                    if (translatedEntity != null)
                    {
                        translatedEntity.LanguageID = languageToTranslate;
                        translatedEntity.MallID = item.MallID;
                    }
